...
widget(Pid) ->
    ...
    gs:frame(packer, Win,[{packer_x, [{stretch,1,50},{stretch,2,150}]},
			  {packer_y, [{stretch,2,150},
				      {stretch,1,25,25}]}]),
    gs:create(editor,editor,packer, [{pack_x,2},{pack_y,1},{vscroll,right}]),
    gs:create(editor,userlist,packer, [{pack_xy,{1,1}},{vscroll,right}]),
...
add_users([H|T]) ->
    gs:config(userlist, {insert,{'end',H}}),    
    gs:config(userlist, {insert,{'end',"\n"}}),
    add_users(T);

add_users([]) ->[].


update_user_list({_,NewList}) ->
    gs:config(userlist, clear),%{delete,{'start','end'}}),
    add_users(NewList).
